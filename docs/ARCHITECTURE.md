...
- **What deletion pseudonymises is `public`, and that is not the same as legal erasure (ADR-0019 decision 1, T11/F1).** `pseudonymise_account` removes personal data from every table this application can reach: no row available to `anon`, `authenticated` or `service_role` reconnects the subject UUID to a person. It does **not** touch Supabase Auth's own history — `auth.audit_log_entries` retains authentication events keyed by that UUID, with request metadata, and deleting the `auth.users` row does not purge them. **`auth` is not in PostgREST's exposed schemas**, so that history has no API surface here at all; it is reachable only by a database superuser or the Supabase dashboard. State the two separately and never collapse them: **technical pseudonymisation** is the mechanism T10 built and tested; **erasure** is a legal conclusion about all processing, including retention periods and lawful bases, and the first does not establish the second. **The retention and purge policy for auth audit history is unresolved and belongs to T37** — no period is stated anywhere in this repository, and inventing one in a doc or a column comment would be a legal claim made by an engineer.
