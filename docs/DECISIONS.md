...
## ADR-0017 — Account deletion is a tombstone profile under the same subject UUID, with `auth.users` genuinely deleted

**Status:** Accepted · T10 · 2026-08-12 · resolves the mechanism ADR-0010 decision 2 deliberately left open · amends `ARCHITECTURE.md` §11.5 on one factual point · changes no T03–T09 privilege, policy or grant
...
- **A surrogate financial subject** — repointing the retained rows at a shared "deleted user" id — requires `UPDATE credit_ledger.user_id`. Not implementable without dismantling the strongest guarantee in the schema. **Rejected.**
- **A nullable financial actor column** — nulling `credit_ledger.user_id` — requires the same `UPDATE`. **Rejected for the same reason**, and it would additionally destroy per-user reconciliation, which §11.4 requires to keep working for closed accounts because a chargeback can arrive months later.
- **A tombstone under the same subject UUID** requires no write to the ledger at all. **Viable, and therefore chosen.**

The ledger's `user_id` value is immutable for the life of the row. The only remaining lever is to make that UUID stop resolving to a person — and the email, the identities and the login all live in `auth.users`.

**Decisions.**

**1. The profile row is retained as a tombstone under the same UUID, scrubbed of every personal field, marked with `deleted_at`.** It exists solely to satisfy the two `ON DELETE RESTRICT` foreign keys. What remains is an id, a balance, three timestamps and the schema defaults of three `NOT NULL` preference columns.

**2. `profiles.id → auth.users(id) ON DELETE CASCADE` is dropped, and the `auth.users` row is genuinely deleted.** The cascade is precisely what makes deletion impossible: it tries to take the profile with it and `ON DELETE RESTRICT` holds it back. Deleting the auth record — rather than scrambling its email in place — is the stronger privacy outcome, because `auth.users` carries identities, provider emails and metadata that a scrub would have to enumerate correctly inside a schema Supabase owns and may change.

**3. The restriction the dropped foreign key provided is re-established one layer up, and restated in its own terms.** A `BEFORE DELETE` trigger on `auth.users` refuses to delete a user whose profile still holds personal data, raising `23503` with a message naming `pseudonymise_account`. **This is the most important object in the migration.** Without it, dropping the FK would turn a loud `23503` into a silent orphan: the auth user gone, nothing cascading, and a profile row full of the person's country, locale, timezone, budget and cost assumptions left behind with no owner and no route by which anyone would notice. The rule is now stated as what it always meant — *an auth user may not be deleted while their profile still holds personal data* — rather than as the proxy "while financial rows exist", which is the wrong test now that the whole point is that financial rows exist afterwards too.

**4. The tombstone invariant is a check constraint, not a promise made by the routine.** `profiles_tombstone_carries_no_personal_data` rejects any write that puts personal data onto a row with `deleted_at` set. A routine that scrubs columns is a promise; a constraint is a guarantee, and it is also where a personal column added by a future task is meant to be noticed — because that column will not be in the scrub list.

**5. `credit_balance` is NOT zeroed on the tombstone.** Zeroing it would falsify the record and break AC10.8's invariant `sum(credit_ledger.delta) = profiles.credit_balance` for exactly the rows §11.4 requires reconciliation to keep covering. The balance is not personal data: it is a number that reconciles against a ledger nobody can trace to a person.

**6. `pseudonymise_account(uuid)` is idempotent and convergent, and it deletes the personal rows explicitly.** Every statement is a `DELETE` or an `UPDATE` to a fixed value, so a retry is a no-op that returns the same summary. **`ARCHITECTURE.md` §11.5 is factually wrong on one point and is amended here:** it says profile, unlocks, watchlist, purchase records and barcode lookups "cascade from `auth.users`". They cascade from `public.profiles`, and the profile now survives — so no cascade fires and relying on the documented-but-untrue one would leave every personal row in place. `app_events.user_id` is nulled explicitly for the same reason.

**7. The retention DURATION is a legal-policy question and is deliberately not encoded.** The mechanism retains indefinitely. Inventing a number here — 6 years, 7 years, "as required" — would be a legal claim made by an engineer, written into a column comment, and inherited as fact. It is recorded as unresolved, in the same register as the rate-card ambiguities `MARKET_PLAYBOOK.md` §4.1 holds for the founder.

**8. Session handling lives in `src/proxy.ts`, not `src/middleware.ts`.** T10 and `src/app/(app)/README.md` both name the latter. Next.js 16 renamed Middleware to Proxy and the file convention is `proxy.ts` (`node_modules/next/dist/docs/01-app/01-getting-started/16-proxy.md`). The functionality is unchanged. Following the framework the repository actually depends on beats following a filename in a task description written before the upgrade.

**9. The proxy is default-deny over an allowlist of public paths, and it is not the authorization boundary.** A route group is a source-tree convention — `(app)/settings/page.tsx` serves `/settings` — so the proxy cannot match on `(app)` and must be given paths some other way. A list of *protected* prefixes fails open the day someone adds a route and forgets the list; an allowlist of *public* paths fails closed, visibly. Separately, and per Next's own guidance that Proxy "should not be used as a full session management or authorization solution", the authoritative check is in `(app)/layout.tsx` and in RLS: a protected page refuses to render for an unauthenticated caller even if the proxy never runs.

**10. Authentication happens in Server Actions, so the session cookie is `httpOnly`.** AC1.4 asks only that tokens are absent from `localStorage`. `@supabase/ssr`'s browser client already satisfies that — it uses cookies — but those cookies are written by JavaScript and are readable by any script on the page. Signing in on the server means the cookie arrives via `Set-Cookie` with `httpOnly`, which client JavaScript cannot read at all. `sameSite` is `lax` rather than `strict` by necessity: the magic-link and verification flows return the user from their mail client as a cross-site top-level navigation, and `strict` would withhold the cookie on exactly that request.

**11. The signup grant is `grant_credits` called with a deterministic, versioned idempotency key (`signup_grant:v1:<user id>`), and it is retried on every pre-onboarding render.** T07 remains the only writer; this adds a key, not a path. Because the key is a pure function of the subject, a failed first attempt is completed by the next page load instead of silently costing the user the five credits AC10.6 promises — and a double-grant is impossible, guaranteed by `credit_ledger.idempotency_key UNIQUE` rather than by this code. **No conflict with T03 was found:** `handle_new_user()` writes only the id, sets no balance and creates no ledger row, so a fresh account reconciles at zero and nothing in T07's accounting is being rewritten.

**Consequences.**

- **`profiles` no longer has a database-enforced link to `auth.users`.** That guarantee is incompatible with retaining financial history under `ON DELETE RESTRICT`. It is replaced by the tombstone constraint, the delete guard, and the fact that no client can create a profile at all — there is no INSERT policy and no INSERT grant, and `handle_new_user` remains the only creation path.
- **A tombstone is permanently unreachable by every client role, without any change to T06.** Its policies are `id = auth.uid()`; a tombstone has no auth user, so no JWT can carry its subject. That is a stronger property than the foreign key provided.
- **Five existing pgTAP assertions were rescoped rather than deleted**, in `schema_a`, `schema_b` and `schema_c`: each asserted that deleting an auth user cascaded the profile away, and each now asserts the post-T10 truth — refused while personal data remains, permitted once tombstoned, profile retained. The two catalogue sweeps and `security_baseline.test.sql`'s function allowlist gained the two new functions. Those suites turning red is them working as designed.
